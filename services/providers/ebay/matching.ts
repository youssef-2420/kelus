...
const accessoryTerms = [
  ...
  "lcd", "digitizer", "box only", "empty box", "dummy", "display model", "housing", "back glass",
];
...
export function isAccessory(item: EbayItemSummary) {
  ...
  return accessoryTerms.some((term) => text.includes(term));
}
...
export function matchesModel(item: EbayItemSummary, product: Product) {
  ...
  if (product.slug === "iphone-17") return !text.includes("iphone 17 pro");
  return false;
}

function storageTokens(storage?: string) {
  if (!storage) return [];
  const normalized = storage.toLowerCase().replace(/\s/g, "");
  if (normalized === "1tb") return ["1tb", "1024gb"];
  return [normalized, normalized.replace("gb", " gb")];
}

export function matchesStorage(item: EbayItemSummary, variant: ProductVariant) {
  const text = ebayItemText(item).replace(/\s/g, "");
  return storageTokens(variant.storage).some((token) => text.includes(token.replace(/\s/g, "")));
}
...
export function isFixedPrice(item: EbayItemSummary) {
  return !item.buyingOptions?.length || item.buyingOptions.includes("FIXED_PRICE");
}
...
export function matchesVariant(item: EbayItemSummary, product: Product, variant: ProductVariant, condition: ConditionFilter) {
  ...
  if (text.includes("locked") && !text.includes("unlocked")) return false;
