...
test("search is the single canonical product discovery experience", async () => {
  ...
  assert.match(home, /ComparisonStage/);
  assert.doesNotMatch(home, /OnboardingTour|desk-coverage-hint/);
  assert.doesNotMatch(home, /LiveShowcase/);
  assert.match(page, /SearchControls minimal minimalAction deferProductSelection focusOnMount/);
  assert.doesNotMatch(page, /ComparisonStage|ProductListingCard|search-featured|search-browse|search-console-kicker|search-console-categories/);
  assert.match(page, /alternates: \{ canonical: "https:\/\/kelus\.me\/search" \}/);
  assert.doesNotMatch(page, /listProductListingPreviews|listHomeComparisonPreviews|categoryHubs|getProductCardStatus/);
...
test("phase 2 UX polish covers sign-in toast and mobile search overlay", async () => {
  ...
  assert.match(styles, /html\.is-search-focus \.home-desk \.desk-pick\{[^}]*scale\(/);
  assert.match(styles, /\.search-overlay-scrim\{[^}]*background:transparent/);
  assert.doesNotMatch(styles, /html\.is-search-focus \.home-desk \.desk-pick\{[^}]*opacity:\.3/);
  ...
  assert.doesNotMatch(styles, /\.search-overlay-scrim\{[^}]*background:rgba\(15,40,35,\.4\)/);
  assert.match(search, /scrollIntoView/);
  assert.match(search, /hero-config-close/);
  assert.match(search, /Opening comparison/);
  assert.match(search, /Choose the exact setup, then compare matching offers/);
  assert.match(search, /configOpen/);
...
test("outstanding UX surfaces explain confidence, history progress, and catalog availability", async () => {
  ...
  assert.doesNotMatch(search, /ProductListingCard|listProductListingPreviews|listHomeComparisonPreviews/);
...
test("kit-guided UX adds skip link, homepage desk, and comparison badges", async () => {
  ...
  assert.doesNotMatch(homeLearn, /home-learn-categories|home-learn-foot|categoryHubs/);
