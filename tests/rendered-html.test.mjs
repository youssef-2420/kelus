...
test("kit-guided UX adds skip link, homepage desk, and comparison badges", async () => {
  ...
  assert.doesNotMatch(comparison, /Live Kelus pick/);
  assert.match(home, /HomeLearnSection/);
  assert.match(home, /desk-coverage-hint/);
  ...
  assert.match(styles, /@keyframes desk-cheapest-recede/);
  assert.match(styles, /@keyframes desk-pick-lock/);
  assert.match(styles, /@keyframes search-finding-ring/);
  assert.doesNotMatch(styles, /home-flow-line 2\.8s[\s\S]*infinite/);
  assert.match(styles, /\.desk-pick-total\.is-cheapest\{[^}]*opacity:\.55/);
  assert.match(styles, /\.desk-pick-total\.is-cheapest strong\{[^}]*color:#b8cdc7/);
  assert.doesNotMatch(comparison, /desk-pick-checks/);
  assert.doesNotMatch(styles, /\.desk-kicker/);
