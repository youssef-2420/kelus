...
function OtherOffer({ offer, productName, initiallyOpen }: { offer: Offer; productName: string; initiallyOpen: boolean }) {
  return <details className="nr-other" open={initiallyOpen}><summary><EbayLogo/><span><b>{productName} · {titleCase(offer.condition)} listing</b><small>{titleCase(offer.condition)} · {offer.shippingCostKnown === false ? "Shipping unavailable" : offer.shippingCost ? `+$${offer.shippingCost} shipping` : "Free shipping"} · {offer.returnPolicy ?? "Return terms unavailable"}</small></span><strong>${knownTotal(offer) ?? offer.price}</strong><Icon name="chevron" size={17}/></summary><div className="nr-other-detail"><ListingImage offer={offer} productName={productName}/><div><p>{offer.seller.feedbackPercentage ? `${offer.seller.feedbackPercentage}% positive · ` : ""}{offer.seller.name}</p><small>{updatedLabel(offer.lastUpdated)} · Live eBay offer</small><span className="nr-cta"><OutboundRetailerCTA offer={offer}/></span></div></div></details>;
}
