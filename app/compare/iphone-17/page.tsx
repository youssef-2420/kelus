    ...
    <section className="compare-page section">
      ...
      <div className="compare-table" role="table">
        <div className="compare-row compare-columns" role="row">
          ...
          {offers.map((offer) => <div role="columnheader" key={offer.id}><small>{offer.id === pick.id ? "Our Pick" : offer.id.includes("ebay") ? "Cheapest" : "Strong retailer option"}</small><b>{offer.retailer.name}</b><strong>{"$"}{offer.price}</strong></div>)}
        </div>
        {rows.map((row) => <div className="compare-row" role="row" key={row[0]}>
          <span role="rowheader">{row[0]}</span>
          {row.slice(1).map((value, index) => <div key={value + index}>{index === 0 && row[0] === "Price" ? <b className="recommended-value"><Icon name="check" size={15}/>{value}</b> : value}</div>)}
        </div>)}
      </div>
      <div className="compare-callout"><Icon name="tag"/><div><b>Why we picked {pick.retailer.name}</b><p>{recommendation.reasons.join(" · ")}</p></div><OutboundRetailerCTA offer={pick}/></div>
    </section>
  </main>;
}
