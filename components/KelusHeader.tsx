...
export function KelusHeader({ shell = "default" }: Props) {
  ...
  return (
    <header className={`site-header${shell === "search" ? " is-search-shell" : ""}`}>
      ...
      <nav aria-label="Main navigation">
        {items.map((item) => <Link key={item.href} href={item.href} className={shell === "search" && item.href === "/search" ? "is-active" : undefined}>{item.label}</Link>)}
      </nav>
