  ...
  return (
    <ol className="today-plan-list">
      {route.allocations.map((allocation, index) => {
        ...
        const reasoningId = `plan-reasoning-${allocation.conceptId.replace(/[^a-zA-Z0-9_-]/g, "-")}`;
        return (
          <motion.li layout={!reduceMotion} key={allocation.conceptId}>
            <button type="button" aria-expanded={open} aria-controls={reasoningId} onClick={() => setOpenId(open ? null : allocation.conceptId)}>
              ...
              <span className="plan-end"><span className="plan-time"><strong>{allocation.minutes}</strong><small>MIN</small></span><span className="plan-disclosure" aria-hidden="true">{open ? "−" : "+"}</span></span>
            </button>
            <AnimatePresence initial={false}>
              {open && concept ? (
                <motion.div id={reasoningId} className="plan-reasoning" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: reduceMotion ? 0.1 : 0.22 }}>
