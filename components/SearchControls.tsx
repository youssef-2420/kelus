...
export function SearchControls({ compact = false, defaultProduct = "" }: Props) {
  ...
  function submit() { window.location.assign("/results?product=" + encodeURIComponent(query || "iPhone 17")); }
  return <form className={compact ? "search-controls compact" : "search-controls"} onSubmit={(event) => { event.preventDefault(); submit(); }}>
    ...
    <div className="search-field location-field"><span>Location</span><div><Icon name="pin" size={19}/><strong>United States</strong></div></div>
