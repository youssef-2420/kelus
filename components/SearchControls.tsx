...
function SuggestionLoading() {
  return <div className="suggestion-loading" role="status" aria-live="polite" aria-label="Finding products">
    {[0, 1, 2].map((item) => <span key={item}><i/><b/><small/></span>)}
  </div>;
}
...
export function SearchControls({ compact = false, minimal = false, minimalAction = false, deferProductSelection = false, initialCriteria = defaultSearch, resultPath, actionLabel = "Search" }: Props) {
  ...
  const suggestionProducts = trimmedQuery ? visibleMatches : featuredMatches;
  ...
  function onKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    ...
    if (event.key === "Enter" && open && activeIndex >= 0 && suggestionProducts[activeIndex]) { event.preventDefault(); chooseProduct(suggestionProducts[activeIndex]); }
  ...
  const suggestionsPanel = open && <div className="suggestions" id={listboxId} role="listbox" aria-label="Product suggestions">
    {loading ? <SuggestionLoading/> : suggestionProducts.length ? <>
      ...
      {showSearchResults && <button type="submit" className="suggestions-footer">View all results for “{query}” <Icon name="arrow" size={16}/></button>}
    </> : trimmedQuery ? <p className="suggestion-state">Kelus does not support this product yet.</p> : null}
  </div>;
