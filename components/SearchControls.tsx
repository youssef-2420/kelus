...
export function SearchControls({ compact = false, minimal = false, minimalAction = false, deferProductSelection = false, initialCriteria = defaultSearch, resultPath = "/results-v2", actionLabel = "Search" }: Props) {
  ...
  async function submit(force = false) {
    ...
    const criteria = validateSearchCriteria({ productSlug: submittedProduct.slug, variantId: submittedVariantId || undefined, condition, market });
    if (!criteria) return;
    const startedAt = performance.now();
    setOpen(false); setSubmittedCriteria(criteria); setSearching(true); setSearchFailed(false); setSearchStatus("resolving_product");
  ...
  </form>{searching && <SearchProgress criteria={submittedCriteria ?? { productSlug: selectedProduct.slug, variantId: variantId || undefined, condition, market }} status={searchStatus} failed={searchFailed} onRetry={() => { setSearchFailed(false); window.setTimeout(() => { void submit(true); }, 0); }} />}</>;
}
