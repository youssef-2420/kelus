...
import { getProductBySlug, getVariantById, getVariantsForProduct, marketOptions, searchProducts } from "@/lib/demo-data";
...
export function SearchControls({ compact = false, minimal = false, initialCriteria = defaultSearch }: Props) {
  ...
  async function submit(force = false) {
    ...
    const compatibleVariantId = currentVariant?.productId === product.id
      ? currentVariant.id
      : getVariantsForProduct(product.id)[0]?.id;
    if (typedProduct && typedProduct.slug !== selectedProduct.slug) {
      setSelectedProduct(typedProduct);
      setVariantId(compatibleVariantId ?? "");
    }
    const criteria: SearchCriteria = { productSlug: product.slug, variantId: compatibleVariantId, condition, market };
