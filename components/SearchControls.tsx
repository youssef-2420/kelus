...
export function SearchControls({ compact = false, minimal = false, initialCriteria = defaultSearch }: Props) {
  ...
  return <><form className={`${compact ? "search-controls compact" : "search-controls"}${attributeLabel ? " has-attribute" : " no-attribute"}${searching ? " is-searching" : ""}`} onSubmit={(event) => { event.preventDefault(); submit(); }} aria-busy={searching}>
    ...
    <button className="button button-primary search-submit" type="submit" disabled={searching}>{searching ? "Checking…" : "Compare prices"}<Icon name={searching ? "search" : "arrow"} size={19}/></button>
