...
import { getSearchAttributeVariants, getVisibleSearchAttributeLabel } from "@/lib/product-attributes";
...
export function SearchControls({ compact = false, minimal = false, minimalAction = false, deferProductSelection = false, initialCriteria = defaultSearch, resultPath = "/results-v2", actionLabel = "Search" }: Props) {
  ...
  function chooseProduct(product: typeof selectedProduct) { setSelectedProduct(product); setProductSelected(true); setVariantId(getSearchAttributeVariants(product, getVariantsForProduct(product.id))[0]?.id ?? ""); setQuery(product.name); setOpen(false); setActiveIndex(-1); trackEvent({ name: "product_selected", productSlug: product.slug }); }
  ...
  async function submit(force = false) {
    ...
    const submittedProduct = productSelected ? selectedProduct : matches[0] ?? selectedProduct;
    const submittedVariantId = productSelected ? variantId : getSearchAttributeVariants(submittedProduct, getVariantsForProduct(submittedProduct.id))[0]?.id ?? "";
