...
import { conditionOptions, getVariantsForProduct, products } from "@/lib/demo-data";
...
export function SearchControls({ compact = false, defaultProduct = "" }: Props) {
  ...
  const selectedProduct = products.find((product) => product.name.toLowerCase() === query.toLowerCase());
  const variantOptions = selectedProduct ? getVariantsForProduct(selectedProduct.id) : [];
  ...
  return <form className={compact ? "search-controls compact" : "search-controls"} onSubmit={(event) => { event.preventDefault(); submit(); }}>
    <label className="search-field product-field"><span>Product</span><div><Icon name="search" size={20}/><input value={query} placeholder="Search for a product" onFocus={() => setOpen(true)} onBlur={() => window.setTimeout(() => setOpen(false), 120)} onChange={(event) => { setQuery(event.target.value); setVariant("Any variant"); setOpen(true); }} /></div>
      {open && results.length > 0 && <div className="suggestions">{results.map((product) => <button type="button" key={product.slug} onMouseDown={() => { setQuery(product.name); setVariant("Any variant"); setOpen(false); }}><ProductMark label={product.image} small/><span><b>{product.name}</b><em>{product.brand} · {product.category}</em></span></button>)}</div>}
    </label>
    ...
    <label className="search-field variant-field"><span>Variant</span><div className="select-control"><select value={variant} onChange={(event) => setVariant(event.target.value)}><option>Any variant</option>{variantOptions.map((item) => <option key={item.id}>{item.label}</option>)}</select><Icon name="chevron" size={17}/></div></label>
