...
export function SearchProgress({ criteria, status, failed, onRetry }: { criteria: SearchCriteria; status: SearchStatus; failed: boolean; onRetry: () => void }) {
  ...
  return <div className="search-transition" role="status" aria-live="polite" aria-atomic="true"><div className="search-progress-card"><div className="search-progress-mark" aria-hidden="true"><span>k</span><i/><i/><i/></div><p className="eyebrow">Kelus comparison</p><h2>{product?.name ?? "Your product"}{variant?.label && <span> · {variant.label}</span>}</h2><div className="search-progress-status"><Icon name={failed ? "close" : "search"} size={18}/><p>{message}</p></div>{failed && <><p className="search-progress-help">Please try again in a moment.</p><button type="button" className="button button-primary" onClick={onRetry}>Try again <Icon name="arrow" size={17}/></button></>}<span className="sr-only">{message}</span></div></div>;
}
