...
export function OutboundRetailerCTA({ offer, compact = false }: { offer: Offer; compact?: boolean }) {
  ...
  return <a className={compact ? "text-link outbound-cta" : "button button-primary outbound-cta"} href={offer.affiliateUrl} target="_blank" rel="noreferrer" onClick={() => trackEvent({ name: "retailer_clicked", offerId: offer.id })}>{label}<Icon name="arrow" size={compact ? 15 : 17}/></a>;
}
