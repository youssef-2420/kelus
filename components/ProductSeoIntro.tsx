...
export function ProductSeoIntro({ product, variant, condition }: ProductSeoIntroProps) {
  ...
  return (
    <section className="product-seo-intro section" aria-label="Product overview">
      ...
      <p className="product-seo-lead">
        Kelus compares validated live eBay offers for the {product.brand} {product.name} ({variant.label}) in {conditionCopy(condition)}.
        Each listing is checked for product match, seller trust, shipping, and return policy so you can see which offer is actually worth buying.
      </p>
    </section>
  );
}
